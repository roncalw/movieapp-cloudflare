...
# Run one saved SQL support script against the remote Cloudflare D1 database.
#
...
#
# Examples:
#   zsh support/run-sql-remote.sh support/sql/tmdb-counts.sql
#   zsh support/run-sql-remote.sh support/sql/movie-search-by-year.sql START_DATE=2020-01-01 END_DATE=2020-12-31
#
# Parameter:
#   $1
...
#   This helper reads the SQL file with cat and passes the SQL text into
#   Wrangler's --command option so SELECT queries print their actual rows.
#
# Why this script uses --remote:
#   --remote tells Wrangler to run the SQL against the real Cloudflare D1
#   database. This is the database the deployed Worker uses.
#
...
#
#   --remote
#     Uses Cloudflare's hosted D1 database instead of local development state.
#
...
if [[ $# -lt 1 ]]; then
  ...
  echo "Usage: support/run-sql-remote.sh support/sql/query-name.sql [TOKEN=value ...]" >&2
  exit 1
fi
...
# Run the saved SQL against remote D1 and print the result in the VS Code terminal.
npx wrangler d1 execute movieapp-db --remote --command="$sql_text"
